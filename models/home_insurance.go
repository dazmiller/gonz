...
func (t *HomeInsurance) TableName() string {
	return "home_insurance"
}

func init() {
	orm.RegisterModel(new(HomeInsurance))
}

// AddHomeInsurance insert a new HomeInsurance into database and returns
// last inserted Id on success.
func AddHomeInsurance(m *HomeInsurance) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetHomeInsuranceById retrieves HomeInsurance by Id. Returns error if
// Id doesn't exist
func GetHomeInsuranceById(id int) (v *HomeInsurance, err error) {
	o := orm.NewOrm()
	v = &HomeInsurance{Id: id}
...
// GetAllHomeInsurance retrieves all HomeInsurance matches certain condition. Returns empty list if
// no records exist
func GetAllHomeInsurance(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(HomeInsurance))
	// query k=v
	for k, v := range query {
...
// UpdateHomeInsurance updates HomeInsurance by Id and returns error if
// the record to be updated doesn't exist
func UpdateHomeInsuranceById(m *HomeInsurance) (err error) {
	o := orm.NewOrm()
	v := HomeInsurance{Id: m.Id}
...
// DeleteHomeInsurance deletes HomeInsurance by Id and returns error if
// the record to be deleted doesn't exist
func DeleteHomeInsurance(id int) (err error) {
	o := orm.NewOrm()
	v := HomeInsurance{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&HomeInsurance{Id: id}); err == nil {
